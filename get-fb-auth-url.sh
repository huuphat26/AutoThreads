#!/bin/bash
# Tạo Facebook OAuth URL để lấy authorization code
# Sử dụng:
#   bash get-fb-auth-url.sh
#   bash get-fb-auth-url.sh --open
...
if [ -z "$CLIENT_ID" ] || [ -z "$REDIRECT_URI" ]; then
  echo "❌ Thiếu FB_APP_ID hoặc FB_REDIRECT_URI trong .env"
  exit 1
fi
...
echo "bash get-fb-token.sh '<callback-url-or-code>'"
